...
// worker.js
var BUTTON_ID = "claim-offer";
var REPORT_TIME_ZONE = "Asia/Dhaka";
var AUTO_CLEANUP_THRESHOLD_BYTES = 100 * 1024 * 1024;
function getReportDate(offsetDays = 0) {
  ...
  const getPart = /* @__PURE__ */ __name((type) => Number(dateParts.find((part) => part.type === type)?.value), "getPart");
  const date = new Date(Date.UTC(getPart("year"), getPart("month") - 1, getPart("day")));
  date.setUTCDate(date.getUTCDate() + offsetDays);
  return date.toISOString().slice(0, 10);
}
__name(getReportDate, "getReportDate");
...
var inMemorySettings = { bannersPublished: false, whatsappNumber: "8801762050353" };
...
__name(readSiteSettings, "readSiteSettings");
...
__name(readSiteBanners, "readSiteBanners");
...
__name(updateSiteBanners, "updateSiteBanners");
async function readClickCount(database) {
  await ensureTables(database);
  const row = await database.prepare("SELECT total_clicks FROM click_stats WHERE button_id = ?").bind(BUTTON_ID).first();
  return Number(row?.total_clicks || 0);
}
__name(readClickCount, "readClickCount");
...
async function readDailyClicks(database, numberOfDays) {
  const result = await database.prepare(`
      ...
      LIMIT ?
    `).bind(numberOfDays).all();
  return result.results || [];
}
__name(readDailyClicks, "readDailyClicks");
...
__name(readCountryClicks, "readCountryClicks");
...
__name(hasValidAdminKey, "hasValidAdminKey");
async function deleteOldDailyClicks(database) {
  const cutoffDate = getReportDate(-6);
  const oldData = await database.prepare("SELECT COUNT(*) AS rows, COALESCE(SUM(total_clicks), 0) AS clicks FROM daily_clicks WHERE click_date < ?").bind(cutoffDate).first();
  const result = await database.prepare("DELETE FROM daily_clicks WHERE click_date < ?").bind(cutoffDate).run();
  const countryResult = await database.prepare("DELETE FROM daily_country_clicks WHERE click_date < ?").bind(cutoffDate).run();
  return {
    cutoffDate,
...
__name(deleteOldDailyClicks, "deleteOldDailyClicks");
...
__name(runAutomaticStorageCleanup, "runAutomaticStorageCleanup");
...
__name(getBooksData, "getBooksData");
var CIPHER_KEY = 90;
var EXPIRY_MS = 1500;
function encodeBookId(id) {
  ...
  const nonce = Math.floor(Math.random() * 65535).toString(36);
  const raw = id + "|" + nonce;
  const bytes = Array.from(new TextEncoder().encode(raw));
  const salt = ts % 127;
  const xorBytes = bytes.map((b, i) => b ^ (CIPHER_KEY ^ salt + i * 7 & 127));
  let binary = "";
  for (let i = 0; i < xorBytes.length; i++) {
    binary += String.fromCharCode(xorBytes[i]);
  }
  const enc = btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  return tsHex + "." + enc;
}
__name(encodeBookId, "encodeBookId");
    ...
    if (ts && !isNaN(ts)) {
      ...
      try {
        ...
        for (let i = 0; i < binary.length; i++) {
          decodedBytes[i] = binary.charCodeAt(i) ^ (CIPHER_KEY ^ salt + i * 7 & 127);
        }
...
__name(decodeBookId, "decodeBookId");
...
__name(hasValidApiKey, "hasValidApiKey");
function extractBookKey(url, booksData) {
  ...
  const cleanPath = url.pathname.replace(/^\/book\//, "").replace(/^\//, "").replace(/\/$/, "").replace(/\.html$/, "");
...
__name(extractBookKey, "extractBookKey");
function resolveCallerBaseUrl(request, url, body = {}) {
  const explicit = url.searchParams.get("site_url") || url.searchParams.get("base_url") || url.searchParams.get("site") || url.searchParams.get("domain") || url.searchParams.get("origin") || url.searchParams.get("target_origin") || body?.site_url || body?.base_url || body?.site || body?.domain || body?.origin || body?.target_origin || request.headers.get("x-site-url") || request.headers.get("x-origin") || request.headers.get("x-target-origin");
  if (explicit) {
...
__name(resolveCallerBaseUrl, "resolveCallerBaseUrl");
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/data-books" || url.pathname === "/api/data-books/" || url.pathname === "/api/data-book" || url.pathname === "/api/tags") {
      ...
      const booksData2 = await getBooksData(env, url.origin);
      const dataBooksOnly = Object.keys(booksData2);
    ...
    if (url.pathname === "/api/books/resolve" || url.pathname === "/api/book-link") {
      ...
      const resolvedKey = booksData2[bookId] ? bookId : booksData2[decodedKey] ? decodedKey : null;
      if (!resolvedKey) {
      ...
      const book = booksData2[resolvedKey];
      return json({
        success: true,
        id: resolvedKey,
        title: book.title,
    ...
    if (url.pathname === "/api/book-info" || url.pathname === "/api/book-details") {
      ...
      const targetKey = validation.bookId || (booksData2[bookId] ? bookId : null);
      if (!targetKey || !booksData2[targetKey]) {
        return json({ error: "Book not found or token expired.", id: bookId }, { status: 404 });
      }
      const book = booksData2[targetKey];
      return json({
        success: true,
        id: targetKey,
        title: book.title,
    ...
    if (url.pathname === "/api/claim-clicks") {
      ...
      try {
        ...
        if (request.method === "POST") {
          const countryCode = /^[A-Z]{2}$/.test(request.cf?.country || "") ? request.cf.country : "XX";
    ...
    if (url.pathname.startsWith("/assets/") || url.pathname.startsWith("/data/") || url.pathname === "/sw.js" || url.pathname === "/robots.txt" || url.pathname === "/manifest.webmanifest" || /\.(css|js|json|png|jpg|jpeg|svg|webp|ico|woff2|map)$/i.test(url.pathname)) {
      return env.ASSETS.fetch(request);
    }
    if (url.pathname === "/list" || url.pathname === "/list/" || url.pathname === "/list/index.html" || url.pathname === "/count" || url.pathname === "/count/" || url.pathname === "/count/index.html") {
      return env.ASSETS.fetch(request);
    }
    ...
    const isDynamicBookRoute = url.pathname.startsWith("/book") || url.searchParams.has("book") || url.searchParams.has("id") || url.searchParams.has("v") || Boolean(booksData && booksData[rawSlug]);
    if (isDynamicBookRoute) {
      ...
      if (bookHtmlRes.ok) {
        if (bookKey && booksData && booksData[bookKey]) {
          ...
          html = html.replace(/<title id="pageTitle">.*?<\/title>/, `<title id="pageTitle">${escapeHtml(title)}</title>`).replace(/<meta name="description" id="metaDescription" content=".*?" \/>/, `<meta name="description" id="metaDescription" content="${escapeHtml(desc)}" />`).replace(/<link rel="canonical" id="canonicalLink" href=".*?" \/>/, `<link rel="canonical" id="canonicalLink" href="${escapeHtml(canonicalUrl)}" />`).replace(/<meta property="og:title" id="ogTitle" content=".*?" \/>/, `<meta property="og:title" id="ogTitle" content="${escapeHtml(title)}" />`).replace(/<meta property="og:description" id="ogDescription" content=".*?" \/>/, `<meta property="og:description" id="ogDescription" content="${escapeHtml(desc)}" />`).replace(/<meta property="og:url" id="ogUrl" content=".*?" \/>/, `<meta property="og:url" id="ogUrl" content="${escapeHtml(canonicalUrl)}" />`).replace(/<meta property="og:image" id="ogImage" content=".*?" \/>/, `<meta property="og:image" id="ogImage" content="${escapeHtml(img)}" />`).replace(/<meta name="twitter:title" id="twitterTitle" content=".*?" \/>/, `<meta name="twitter:title" id="twitterTitle" content="${escapeHtml(title)}" />`).replace(/<meta name="twitter:description" id="twitterDescription" content=".*?" \/>/, `<meta name="twitter:description" id="twitterDescription" content="${escapeHtml(desc)}" />`);
